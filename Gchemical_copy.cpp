...
int Gchemical::Potential(float* eta[],float* d_eta[],int *gs)
{
int i,j,k,v1,g1;
...
int Gchemical::Miu_P246(float* eta[],float* d_eta[],int *gs)
{
	int g1,v1,i,j,k,indx1,indx2;
	float e1,e2,temp;
	float x_Ni,x_Pt;
...
//							G_p=a[0]*((x_Ni-x1)*(x_Ni-x1)+(x_Pt-x2)*(x_Pt-x2)+ic*(x_Ni-x1)*(x_Pt-x2))+G_p0;
							G_p=a[0]*(2.035*(x_Ni-0.5425)*(x_Ni-0.5425)+0.246*(x_Pt-0.27)*(x_Pt-0.27)-0.0025);
...
//							d_eta[v1][indx2]+=(1-hs)*(a[3]+a[5]*x_Ni+2*a[6]*x_Pt)+hs*(2*a[0]*(x_Pt-x2)+a[0]*ic*(x_Ni-x1));
							d_eta[v1][indx2]+=(1-hs)*(2*a[1]*0.246*(x_Pt-0.20))+hs*(2*a[0]*0.246*(x_Pt-0.27));
							break;
						default:
